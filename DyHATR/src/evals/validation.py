...
def cross_validation(embed, score_type, n_trials=2):
	...
	#print('Trial splits: ', len(trial_splits),len(trial_splits[0]))
	...
	test_pos = test_edges[test_edges[:,2]==1,:]
	test_neg = test_edges[test_edges[:,2]==0,:]

	print('Number of pos and neg samples: ', len(train_pos), len(train_neg), len(test_pos), len(test_neg))
